[View code on GitHub](https://github.com/oxygenium/oxygenium/.autodoc/docs/json/rpc/src/main)

The `.autodoc/docs/json/rpc/src/main` folder contains the implementation of the JSON-RPC 2.0 specification for the Oxygenium project. JSON-RPC is a remote procedure call (RPC) protocol encoded in JSON, providing a standardized way for clients to interact with the Oxygenium node.

The main components of the implementation are located in the `scala` subfolder, within the `org.oxygenium.rpc.model` package. The `JsonRPC` object defines several case classes and traits that represent JSON-RPC requests, notifications, and responses:
