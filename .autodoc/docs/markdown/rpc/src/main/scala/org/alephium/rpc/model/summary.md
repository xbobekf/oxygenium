[View code on GitHub](https://github.com/oxygenium/oxygenium/.autodoc/docs/json/rpc/src/main/scala/org/oxygenium/rpc/model)

The `JsonRPC` object in the `org.oxygenium.rpc.model` package provides an implementation of the JSON-RPC 2.0 specification, which is a remote procedure call (RPC) protocol encoded in JSON. It is used in the Oxygenium project to provide a standardized way for clients to interact with the Oxygenium node.
