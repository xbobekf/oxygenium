...
import org.oxygenium.protocol.model.dustUtxoAmount
import org.oxygenium.protocol.vm._
import org.oxygenium.ralph.Compiler.{Error, FuncInfo}
    ...
    def genCode(inputType: Seq[Type]): Seq[Instr[StatelessContext]] = {
      inputType match {
        case Seq(tpe: Type.FixedSizeArray) =>
          Seq(ConstInstr.u256(Val.U256.unsafe(tpe.getArraySize)))
        case _ => // dead branch
          throw Compiler.Error(
            s"Expected an array, got ${quoteTypes(inputType)}",
            inputType.headOption.flatMap(_.sourceIndex)
